...
class AdvertController extends Controller
{
    ...
    public function newAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if(isset( $upload1 )){
                $image1 = new Image();
                $image1->setAlt( $advert->getTitle() ."_1" );
                $image1->setUrl( $upload1 );
                $advert->addImage($image1);
            }
            if(isset( $upload2 )){
                $image2 = new Image();
                $image2->setAlt( $advert->getTitle() ."_2" );
                $image2->setUrl( $upload2 );
                $advert->addImage($image2);
            }
            if(isset( $upload3 )){
                $image3 = new Image();
                $image3->setAlt( $advert->getTitle() ."_3" );
                $image3->setUrl( $upload3 );
                $advert->addImage($image3);
            }
            //dump($advert->getImages());

            $advert->setDate( new \DateTime( date("Y-m-d H:i:s") ) );
            //die();
            $em = $this->getDoctrine()->getManager();
            $em->persist($advert);
            $em->flush();
